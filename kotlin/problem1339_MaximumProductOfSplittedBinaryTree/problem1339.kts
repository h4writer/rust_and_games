...
class Solution {
    ...
    fun subtree_sum(node: TreeNode?): Int {
        //        21
        //    11       9
        //  4    5       6
        if (node == null)
        ...
        val sumL = subtree_sum(node.left);
        val sumR = subtree_sum(node.right);
        node.`val` += sumL + sumR;
        return node.`val`;
    }

    fun follow_closest_to(twice_wanted: Int, node: TreeNode): Int {
        var closest: Int = node.`val`;

        if (node.right != null) {
            val child = node.right!!.`val`;
            if (abs(2*child - twice_wanted) <= abs(2*closest - twice_wanted)) {
                closest = follow_closest_to(twice_wanted, node.right!!);
            }
        }

        if (node.left != null) {
            val child = node.left!!.`val`
            if (abs(2*child - twice_wanted) <= abs(2*closest - twice_wanted)) {
                closest =  follow_closest_to(twice_wanted, node.left!!);
            }
        }
    ...
    fun maxProduct(root: TreeNode?): Int {
        ...
        // preprocess: make it a subtree sum.
        val sum = subtree_sum(root);

        // Maximum product is values closest to sum/2.
        var closest = follow_closest_to(sum, root);

        var res =  closest.toLong() * (sum - closest);
...
fun createTree(tree: Array<Int?>): TreeNode? {
    ...
    while (i < tree.size) {
        var left = tree[i++];
        var right = if (i < tree.size) tree[i++] else null;
